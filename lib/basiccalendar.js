...
function editorCancel() {
    currentEditField.innerHTML = editorOriginalValue;
    closeCurrentEditor();
}
...
function buildCal(d, m, y, cM, cH, cDW, cD, brdr, mode) {
    ...
    t+= '</tr><tr><td colspan="7" class="boutton"><a href="javascript:calendarCancel()">Annuler</a></td></table></div>';
    return t;
}
...
function editDate(fieldId) {
    ...
    if(currentEditor !== "") {
        ...
        scroll(0, posy);
        var curyear = editorOriginalValue.substring(4, 8)*1;
        var curmonth = editorOriginalValue.substring(2, 4)*1;
        var curday = editorOriginalValue.substring(0, 2)*1;
        if (editorOriginalValue.length<8){
        	var curyear = todaydate.getFullYear()*1;
        	var curmonth = todaydate.getMonth()*1;
        	var curday = todaydate.getDay()*1;
        }
